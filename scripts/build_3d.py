...
from model_config import make_config
C=make_config()
from cabinetry import special_cabinet
...
scene.render.resolution_y = 1866
...
for name, color in C['materials'].items():
    ...
    if name=='metal': p.inputs['Metallic'].default_value = .8
    if name=='black': p.inputs['Roughness'].default_value = .35
    if name=='glass':
        p.inputs['Transmission Weight'].default_value = 1.0
        p.inputs['Roughness'].default_value = .12
        p.inputs['Alpha'].default_value = 1.0
        p.inputs['Base Color'].default_value = (.92,.97,.98,1)
...
for name,b in C['walls'].items():
    parts=[b]
    if not C.get('walls_are_final_segments'):
...
for name,op in C['openings'].items():
    ...
    elif kind in ('removed_door', 'connected'):
        # Open boundaries have no inferred lintel. Only explicitly specified upper solids are built.
        spec=op.get('upper_solid')
        if spec is not None:
            assert spec.get('basis'), 'Explicit upper solid requires a recorded basis'
            bottom=spec['bottom']; top=spec['top']
            assert 0 <= bottom < top <= H
            box(name+'_header',b,bottom,top-bottom,'warm_white','Headers_Concept')
    elif kind in ('door', 'glass_door', 'double_glass_door'):
        box(name+'_header',b,DH,H-DH,'warm_white','Headers_Concept')
    else:
        raise ValueError('Unsupported opening kind: '+kind)
...
for name,f in C['furniture'].items():
    ...
    if special_cabinet(name,f,globals()):
        pass
    elif kind=='bed':
        ...
        box(name+'_top',b,h-.045,.045,mat,room,.012)
        if name.startswith('table'):
            for i,xx in enumerate((x+.015,x+w-.085)):
                box(name+f'_support{i}',[xx,y+d/2-.06,.07,.12],.03,h-.075,mat,room,.004)
        ...
        east=f['front']=='east'
        for i in range(2):
            xx=x+w-.024-i*.022 if east else x+.004+i*.022
            dd=(d-.024)/2
            panel=box(name+f'_front{i}',[xx,y+.018+i*(d/2-.024),.018,dd],.13,h-.16,mat,room,.003)
            # Recessed dark handle remains within the nominal wardrobe footprint.
            box(name+f'_recess{i}',[xx+.016 if east else xx-.001,y+.10+i*(d/2-.024),.002,.12],1.05,.14,'black',room,.001)
    elif kind=='floor_lamp':
        cylinder(name+'_base',(x+w/2,y+d/2,.035),w/2,.05,'black',room)
        cylinder(name+'_pole',(x+w/2,y+d/2,.72),.012,1.38,'metal',room)
        cylinder(name+'_shade',(x+w/2,y+d/2,h-.16),w/2,.28,'linen',room)
    elif kind=='microwave':
        box(name+'_body',[x,y,w,d-.025],z,h,mat,room,.014)
        box(name+'_door_leaf',[x,y+d-.025,w,.025],z,h,'black',room,.008)
        box(name+'_glass',[x+.025,y+d-.004,w-.15,.003],z+.045,h-.09,'glass',room,.006)
        box(name+'_handle',[x+w-.11,y+d-.024,.018,.024],z+.055,.20,'metal',room,.006)
        box(name+'_display',[x+w-.075,y+d-.005,.045,.005],z+.22,.05,'metal',room,.003)
    elif kind in ('cabinet','tower','fridge'):
        box(name+'_carcass',b,.10,h-.13 if h<=.95 else h-.1,mat if kind!='fridge' else 'warm_white',room,.008)
        ...
        if h<=.95:box(name+'_counter',b,h-.03,.03,'stone',room,.01)
    ...
    if f.get('rotation_deg'):
        ...
        for o in root.children:o.location-=center
        root.location=center;root.rotation_euler.z=math.radians(f['rotation_deg'])
...
o=box('coffee_machine',[x+.12,y+.15,.32,.36],.9,.39,'metal',room,.025);metadata(o,**C['equipment']['coffee_machine'])
box('coffee_machine_front',[x+.14,y+.51,.28,.018],1.01,.20,'black',room,.008)
box('coffee_drip_tray',[x+.14,y+.48,.28,.1],.915,.025,'black',room,.008)
box('coffee_tank',[x+.12,y+.12,.32,.07],.95,.30,'glass',room,.015)
cylinder('coffee_spout',(x+.29,y+.52,1.02),.025,.10,'metal',room)
o=box('grinder',[x+.53,y+.2,.18,.23],.9,.27,'black',room,.02);metadata(o,**C['equipment']['grinder'])
cylinder('grinder_hopper',(x+.62,y+.31,1.26),.085,.18,'glass',room)
for i in range(2):
    # Hollow cup made from concentric side faces.
    bpy.ops.mesh.primitive_torus_add(major_radius=.034,minor_radius=.004,major_segments=24,minor_segments=8,location=(x+.9+i*.12,y+.3,.974))
    place(bpy.context.object,f'cup_rim{i}',room,'warm_white')
    cylinder(f'cup{i}',(x+.9+i*.12,y+.3,.935),.036,.07,'warm_white',room)
box('coffee_cup_storage',[x+.12,y,.92,.18],1.65,.045,'walnut',room,.008)
...
table=C['furniture']['table4']['box']
for i,xx in enumerate((table[0]+table[2]*.3,table[0]+table[2]*.7)):
    cylinder(f'pendant_rod{i}',(xx,table[1]+table[3]/2,2.45),.012,.65,'black',room='Lighting')
    cylinder(f'pendant_shade{i}',(xx,table[1]+table[3]/2,2.08),.22,.08,'walnut',room='Lighting')
    cylinder(f'pendant_diffuser{i}',(xx,table[1]+table[3]/2,2.035),.18,.015,'warm_white',room='Lighting')
for name,spec in C['cameras'].items():
    data=bpy.data.cameras.new(name);data.lens=spec.get('lens',22);data.clip_end=200
...
# Complete doors, floors, fixtures and lighting using the current configuration.
sys.path.insert(0,str(ROOT))

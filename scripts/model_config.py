"""Derive Blender metres from the sole R10.5 millimetre layout; no historical model input."""
...
def make_config():
 ...
 def swing(v):
  ...
  return min(cand)[1]
 c={k:s[k] for k in ['defaults','materials','cameras','equipment']};c.update(revision='R10.5',walls_are_final_segments=True,layout_sha256=hashlib.sha256((ROOT/'data/layout.json').read_bytes()).hexdigest())
 ...
 for f in c['furniture'].values():
  for key in ['seat_height_mm','seat_thickness_mm','back_thickness_mm','back_bottom_mm','mattress_top_mm','under_table_clear_height_mm']:
   if key in f:f[key[:-3]]=f[key]/1000
  if 'mattress_size_mm' in f:f['mattress_size']=m(f['mattress_size_mm'])

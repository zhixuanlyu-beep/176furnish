...
def inspect_stored_pdf():
    ...
    if protected:
        message='R4的10页A3横向PDF生成内容已通过检查。保存后的文件现为本机安全软件保护格式，当前程序无法作为普通PDF读取；未尝试移除或绕过保护。请使用组织允许的阅读方式，或先阅读方案册.html和SVG/CSV。未验证此受保护文件在其他设备可打开。\n'
    else:
        message='R4的10页A3横向PDF生成内容已通过检查。本次文件状态检查时为普通PDF；记录见verification.json。\n'
...
def main():
    for path in HERE.glob('*.svg'):
        ET.parse(path)
    counts = {}
    for name, expected in [('家具尺寸表.csv',13),('设备预留表.csv',11),('水电点位表.csv',15),('现场核验表.csv',13),('新图面积标注.csv',11),('新图尺寸标注.csv',7),('电器上下水表.csv',13)]:
    ...
    for prefix,count in [('K',4),('C',3),('L',2),('R',1),('S',2),('H',3),('M',4),('J',4)]:
        for n in range(1,count+1):
            assert f'{prefix}{n:02d}' in doc
    assert len(re.findall(r'<section class="page"',doc))==10
    ...
    sheet=Image.new('RGB',(1200,5*445),'#e6e9e3')
    for n,p in enumerate(pdf):
    ...
    report={'status':'passed','scope':'Document rendering and consistency only; no measured clearances, structural, utility or installation approval.',
            'pdf_pages':10,'revision':'R4','pdf_file':PDF_PATH.name,'pdf_storage':storage_status,'pdf_page_format':'A3 landscape','svg_parse':'5 passed','csv_counts':counts,'page_layout':layout,'mobile_width':mobile,'browser_errors':errors}

...
def inspect_stored_pdf():
    ...
    if protected:
        message='R7的13页A3横向PDF生成内容已通过检查。保存后的文件现为本机安全软件保护格式，当前程序无法作为普通PDF读取；未尝试移除或绕过保护。请使用组织允许的阅读方式，或先阅读方案册.html和SVG/CSV。未验证此受保护文件在其他设备可打开。\n'
    else:
        message='R7的13页A3横向PDF生成内容已通过检查。本次文件状态检查时为普通PDF；记录见verification.json。\n'
...
def main():
    ...
    assert len(re.findall(r'<section class="page"',doc))==13
    ...
    report={'status':'passed','scope':'Document rendering and consistency only; no measured clearances, structural, utility or installation approval.',
            'pdf_pages':13,'revision':'R7','pdf_file':PDF_PATH.name,'pdf_storage':storage_status,'pdf_page_format':'A3 landscape','svg_parse':'8 passed','csv_counts':counts,'page_layout':layout,'mobile_width':mobile,'browser_errors':errors}

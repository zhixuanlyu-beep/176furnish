...
def inspect_stored_pdf():
    ...
    if protected:
        message='R6的12页A3横向PDF生成内容已通过检查。保存后的文件现为本机安全软件保护格式，当前程序无法作为普通PDF读取；未尝试移除或绕过保护。请使用组织允许的阅读方式，或先阅读方案册.html和SVG/CSV。未验证此受保护文件在其他设备可打开。\n'
    else:
        message='R6的12页A3横向PDF生成内容已通过检查。本次文件状态检查时为普通PDF；记录见verification.json。\n'
...
def main():
    for path in HERE.glob('*.svg'):
        ET.parse(path)
    counts = {}
    for name, expected in [('家具尺寸表.csv',16),('设备预留表.csv',12),('水电点位表.csv',17),('现场核验表.csv',14),('新图面积标注.csv',11),('新图尺寸标注.csv',7),('电器上下水表.csv',14)]:
    ...
    assert len(re.findall(r'<section class="page"',doc))==12
    ...
    report={'status':'passed','scope':'Document rendering and consistency only; no measured clearances, structural, utility or installation approval.',
            'pdf_pages':12,'revision':'R6','pdf_file':PDF_PATH.name,'pdf_storage':storage_status,'pdf_page_format':'A3 landscape','svg_parse':'7 passed','csv_counts':counts,'page_layout':layout,'mobile_width':mobile,'browser_errors':errors}

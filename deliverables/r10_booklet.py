"""Publish all R10 drawings and schedules from one geometry/data model."""
import csv
import html
import json
...
def family_svg():
    p=g.start('R10 家庭厅 · AC05在A门洞西侧，书柜集中西侧',740)
...
def build(data):
    ...
    schedules=[
      ...
      ('水电点位表.csv',['点号','候选区域','用途','给排水要求','电气及控制要求','检修及限制','水平定位_mm','标高_mm','回路及保护','状态'],[r+['统一模型估读；待实测','待设备/柜图锁定','待负荷计算','R10概念预留'] for r in points]),
    ...
    svgs={'01-furniture.svg':g.floorplan('家具平面'),'02-alterations-review.svg':g.floorplan('拆改'),
      ...
      '13-island-water-section.svg':g.water_svg()}
    ...
    def page(title,sub,body):
        ...
        pages.append(f'<section class="page" id="p{n:02d}"><header><b>丽水嘉园 · 176㎡ / LISHUI JIAYUAN</b><span>R10 · 2026.09.09</span></header><h1><b>{n:02d}</b>{title}</h1><p class="subtitle">{sub}</p>{body}<footer><span>原图估读 · 未实测 · 非施工图 · 不用于下单、拆墙或预埋</span><span>R10 / {n:02d}</span></footer></section>')
    ...
    common=[('已确定布局','南墙整排柜保持冰箱950–1000、必配独立蒸箱与独立烤箱高柜600、咖啡1300、食品余量的顺序。岛1000×750在东，桌1600×800在西并向西延伸1800。'),
      ('使用边界','北侧为去上部厨房和阳台B主路线。南排就座与蒸烤/咖啡取物须错时。就座状态携篮需暂停岛北操作；全拉椅状态需暂停洗碗装卸。'),
    ...
    page('横向带水岛桌与南侧风管机','R10评审方案：先核底图，再核布局、设备与管线。','<div class="plan-grid"><div class="drawing">'+svgs['01-furniture.svg']+'</div><div>'+cards(common)+'</div></div>')
    details={
      ...
      '04-cabinet-access.svg':('南墙柜立面',[('J01 / J02','主水槽邻柜阀电可达，存水弯与滤芯独立检修。冰箱、上下两台蒸烤分别拆出；热盘依次取，不跨下层热门。'),('J03 / J04','阳台B洗烘与基站按机型留阀门、过滤器、尘袋和整机前抽。狭长净宽、门窗开启及搬出尚未确认。')]),
      ...
      '09-workflows.svg':('北侧通行与七类工作顺序',[('携篮条件','600方形包络计算至阳台B入口内侧，原门洞850估读；柜椅不需移动。进门后关门、转向洗烘及搬机需实物验证。就座时暂停岛北操作，全拉椅时暂停洗碗装卸。')]),
    ...
    for state in g.trial_metrics()['states']:
        state_rows.append([str(state['seats'])+'人'+('全拉椅' if state['pulled'] else '就座'),
          '固定柜/门无相交','北带'+('650' if state['pulled'] else '1000'),
          '、'.join(sorted({k for _,k in state['chair_operator_conflicts']})),
          '、'.join(state['basket_operator_conflicts']) or '无'])
    ...
    for filename,headers,rows in schedules:
        ...
        for i in range(0,len(rows),size):
            page(filename.removesuffix('.csv')+f' · {i//size+1}','配套CSV保留型号、测量、签认与日期填写栏；全部使用R10布局。',table(headers,[[str(v) for v in r] for r in rows[i:i+size]],'compact'))
    css=(HERE/'booklet.css').read_text(encoding='utf-8')
    doc='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>丽水嘉园176㎡ R10</title><style>'+css+'</style></head><body><nav><button onclick="window.print()">打印 / 另存PDF</button>'+''.join(f'<a href="#p{i:02d}">{i:02d} {html.escape(t)}</a>' for i,t in enumerate(names))+'</nav><main>'+''.join(pages)+'</main></body></html>'
    (HERE/'方案册.html').write_text(doc,encoding='utf-8',newline='\n')
    intro='''# 丽水嘉园176㎡ · R10 横向带水岛桌与南侧风管机
...
'''
    ...
    for path,prefix in [(HERE.parent/'README.md','deliverables/'),(HERE/'README.md','')]:
        path.write_text(intro+'\n![R10平面]('+prefix+'preview-furniture.png)\n\n'+'\n'.join(f'- [{title}]({prefix}{name})' for name,title in entries)+'\n'+instructions,encoding='utf-8',newline='\n')
    print(json.dumps({'revision':'R10','pages':len(pages),'svg_count':len(svgs),'model':g.model_digest()},ensure_ascii=False))
